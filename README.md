...
Este proyecto es un backend en Java, utilizando Spring Boot para una API RESTful. La arquitectura sigue un patrón en capas basado en MVC, donde:

- Modelo (Model): Representa datos y lógica de negocio.
- Vista (View): No se implementa; los controladores devuelven respuestas JSON.
- Controlador (Controller): Maneja solicitudes HTTP y coordina con el modelo.

Se extiende a capas para mayor modularidad:

- Capa de Presentación (Controllers): Expone endpoints de la API.
- Capa de Servicio (Services): Lógica de negocio.
- Capa de Acceso a Datos (Repositories): Interactúa con la base de datos.
- Modelos (Models): Entidades de la base de datos.
- DTOs (Data Transfer Objects): Objetos para transferir datos entre capas.
- Migraciones (Migrations): Scripts para cambios en el esquema de la base de datos.

Esta estructura separa responsabilidades, facilitando mantenimiento y pruebas. Una solicitud fluye desde el controlador al servicio, repositorio y base de datos, con DTOs para respuestas.

## Encarpetado

- Controllers: Manejan rutas API y procesan solicitudes/respuestas.
- Services: Implementan lógica de negocio (validaciones, cálculos).
- Repositories: Abstraen acceso a datos (usando JPA/Hibernate).
- Models: Clases entidad (@Entity).
- DTOs: Objetos simples para transferencia de datos.
- Migrations: Scripts versionados para esquemas de BD.

## Endpoints

Todos los endpoints usan el prefijo /api, seguido del controlador y endpoint. Ejemplo: __GET http://localhost:8080/api/users__ (obtener usuarios). O, __POST http://localhost:8080/api/questionnaires__ (crear cuestionarios).

_PS: Para más detalles, revisa los archivos del proyecto._
...
<div align="center">
  <img src="./assets/hf-icon.png" alt="Haroldo-Finanzas" width="360" />
</div>
